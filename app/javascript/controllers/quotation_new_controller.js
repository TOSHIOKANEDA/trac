...
export default class extends BaseController {
  ...
  connect() {
    ...
    // 外国通貨フィールドの表示状態を初期化
    this.initializeCurrencyFieldsVisibility()
  }

  // 外国通貨フィールドの表示/非表示の初期化
  initializeCurrencyFieldsVisibility() {
    const checkbox = this.element.querySelector('.issued-by-foreign-currency-check')
    const container = this.element.querySelector('#currency-fields-container')
  ...
  // 外国通貨チェックボックスの変更時に呼ばれる
  toggleCurrencyFields(event) {
  ...
  addClientRow(event) {
    ...
    newRow.innerHTML = `
      ...
      <td>
        <select name="quotation[quotation_items_attributes][${newIndex}][unit]" class="item-input unit-select">
          ${unitOptionsHtml}
        </select>
      </td>
      <td class="tax-cell">
  ...
  addCompanyRow(event) {
    const button = event.currentTarget
    const companyType = button.dataset.companyType // "carrier", "custom", "agent", "other"
    const newIndex = new Date().getTime()
    const companyOptions = this.getCompanyOptions(companyType)
  ...
  // ヘルパー: ベンダーオプションを取得（タイプ別）
  getCompanyOptions(companyType) {
    const elementId = `${companyType}-options`
    const optionsContainer = document.querySelector(`#${elementId}`)
    if (optionsContainer) {
      return optionsContainer.innerHTML
    }
    return ''
